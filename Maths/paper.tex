...
\item I usually use brackets $[a=b] \in \{0,1\}$ for the boolean indicator function of some expression. An alternative notation is $\mathbb I(a=b)$, or the  \Def{Kronecker} symbol $\d_{ab}$.
...
\item A function $f:\RRR\to\RRR$ is \Def{differentiable} at $x$ if $f'(x) = \lim_{h\to 0} \frac{f(x+h) - f(x)}{h}$ exists. Note, differentiable $\To$ continuous.
...
\begin{items}
\item Monomials, which form the basis of polynomials.
...
\subsubsection{Least Squares \& Gauss-Newton: a very important special case}

A special case that appears a lot in intelligent systems is the
\Def{Least Squares} case: Consider an objective function of the form
\begin{align}
f(x) = \phi(x)^\T \phi(x) = \sum_i \phi_i(x)^2
\end{align}
where we call $\phi(x)$ the \textbf{cost features}. This is also
called a \Def{sum-of-squares} problem. We have
\begin{align}
\na f(x)
...
In this setting, convince yourself of the following: Ideally each
search direction should be orthogonal to the previous one---but not
orthogonal in the conventional Euclidean sense, but orthogonal w.r.t.\
the Hessian $H$. Two vectors $d$ and $d'$ are called \Def{conjugate}
w.r.t.\ a metric $H$ iff $d'^\T H d = 0$. Therefore, subsequent search
directions should be conjugate to each other.
...
\item For arbitrary quadratic functions CG converges in $n$ iterations. But
 this only works with \Def{perfect line search}.
\end{itemize}
